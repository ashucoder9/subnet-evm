...
func TestRun(t *testing.T) {
	tests := map[string]testutils.PrecompileTest{
		...
		"calling increaseCounter from NoRole should fail": {
			Caller:     allowlist.TestNoRoleAddr,
			BeforeHook: allowlist.SetDefaultRoles(Module.Address),
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				...
				input, err := PackIncreaseCounter(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: IncreaseCounterGasCost,
			ReadOnly:    false,
			ExpectedErr: ErrCannotIncreaseCounter.Error(),
		},
		"calling increaseCounter from Enabled should succeed": {
			Caller:     allowlist.TestEnabledAddr,
			BeforeHook: allowlist.SetDefaultRoles(Module.Address),
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
		...
		"calling increaseCounter from Admin should succeed": {
			Caller:     allowlist.TestAdminAddr,
			BeforeHook: allowlist.SetDefaultRoles(Module.Address),
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
		...
		"readOnly increaseCounter should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// set test input to a value here
		...
		"insufficient gas for increaseCounter should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// set test input to a value here
			...
			SuppliedGas: IncreaseCounterGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
	}
